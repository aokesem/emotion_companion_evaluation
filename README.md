...
CHARLS 缺失码如 97/98/99/997/998/999；

性别、城乡、学历、婚姻、生活满意度等分类编码；

15 类慢性病字段；体力活动、社会活动、互联网使用；

前两名子女的信息与联系频率；农业劳动、退休、工作天数等。
...
输入为/用户画像数据/用户画像_抽样500_事实文本.csv。其中包括sample_pick_order，ID以及九大类用户信息。
具体的提示词在数据生成代码\pipeline_config.json中，启用\数据生成代码\generate_sim_user_pipeline.py会启动流程。
事实文本 CSV
   |
   v
步骤 1：用户性格
...
| 生成步骤 | 生成内容   | 直接引用的事实文本                        | 同时引用的上游生成结果          |
| ---- | ------ | -------------------------------- | -------------------- |
| 步骤 1 | `用户性格` | 基础、教育、婚姻、情绪、活动、上网、工作             | 无                    |
| 步骤 2 | `个人经历` | 基础、教育、婚姻、健康、工作、情绪、活动、子女、上网，即全部九类 | `用户性格`               |
| 步骤 3 | `生活烦恼` | 全部九类事实文本                         | `用户性格`、`个人经历`        |
| 步骤 4 | `对话锚点` | 不再直接传原始事实文本                      | `用户性格`、`个人经历`、`生活烦恼` |


最终，每个成功生成的唯一 ID 会写入：用户画像数据/模拟用户信息_500.jsonl